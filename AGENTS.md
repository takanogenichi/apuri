...
- **Apple Silicon (arm64) の前提**: Android SDK の build-tools（`aapt2` 等）は公式に Linux x86_64 版のみ提供されるため、`al` は `platform: linux/amd64` で動作する（`.devcontainer/docker-compose.yml`）。Docker Desktop の Rosetta エミュレーション（Settings → General →「Use Rosetta for x86_64/amd64 emulation on Apple Silicon」）を ON にすること。エミュレーションのため Gradle ビルドはやや遅くなる。
